...
let mk_a f =
  "-a", Arg.Unit f, " Build a library", Arg.Status.empty
...
let mk_annot f =
  ...
  { Arg.Status.empty with deprecated_version = Some "TODO" }
...
let mk_c f =
  "-c", Arg.Unit f, " Compile only (do not link)", Arg.Status.empty
...
let mk_ccopt f =
  "-ccopt", Arg.String f,
  "<opt>  Pass option <opt> to the C compiler and linker", Arg.Status.empty

let mk_clambda_checks f =
  "-clambda-checks", Arg.Unit f, " Instrument clambda code with closure and \
    field access checks (for debugging the compiler)", Arg.Status.empty

let mk_cmi_file f =
  "-cmi-file", Arg.String f,
    "<file>  Use the <file> interface file to type-check", Arg.Status.empty
...
let mk_custom f =
  "-custom", Arg.Unit f, " Link in custom mode", Arg.Status.empty

let mk_dllib f =
  "-dllib", Arg.String f, "<lib>  Use the dynamically-loaded library <lib>", Arg.Status.empty

let mk_dllpath f =
  "-dllpath", Arg.String f,
  "<dir>  Add <dir> to the run-time search path for shared libraries", Arg.Status.empty

let mk_eval f =
  "-e", Arg.String f,
  "<script>  Evaluate given script", Arg.Status.empty

let mk_function_sections f =
  if Config.function_sections then
    "-function-sections",  Arg.Unit f,
    " Generate each function in a separate section if target supports it", Arg.Status.empty
  else
    let err () =
      raise (Arg.Bad "OCaml has been configured without support for \
                      -function-sections")
    in
    "-function-sections", Arg.Unit err, " (option not available)", Arg.Status.empty
...
let mk_stop_after ~native f =
  ...
  " Stop after the given compilation pass.", Arg.Status.empty
...
let mk_save_ir_after ~native f =
  ...
  " Save intermediate representation after the given compilation pass\
    (may be specified more than once).", Arg.Status.empty

let mk_dtypes f =
  "-dtypes", Arg.Unit f, " (deprecated) same as -annot", { Arg.Status.empty with deprecated_version = Some "TODO" }

let mk_for_pack_byt f =
  "-for-pack", Arg.String f,
  "<ident>  Generate code that can later be `packed' with\n\
  \     ocamlc -pack -o <ident>.cmo", Arg.Status.empty

let mk_for_pack_opt f =
  "-for-pack", Arg.String f,
  "<ident>  Generate code that can later be `packed' with\n\
  \     ocamlopt -pack -o <ident>.cmx", Arg.Status.empty

let mk_g_byt f =
  "-g", Arg.Unit f, " Save debugging information", Arg.Status.empty

let mk_g_opt f =
  "-g", Arg.Unit f, " Record debugging information for exception backtrace", Arg.Status.empty

let mk_no_g f =
  "-no-g", Arg.Unit f,
  " Do not record debugging information (default)", Arg.Status.empty

let mk_i f =
  "-i", Arg.Unit f, " Print inferred interface", Arg.Status.empty

let mk_I f =
  "-I", Arg.String f, "<dir>  Add <dir> to the list of include directories", Arg.Status.empty

let mk_impl f =
  "-impl", Arg.String f, "<file>  Compile <file> as a .ml file", Arg.Status.empty

let mk_init f =
  "-init", Arg.String f, "<file>  Load <file> instead of default init file", Arg.Status.empty

let mk_inline f =
  "-inline", Arg.String f,
    Printf.sprintf "<n>|<round>=<n>[,...]  Aggressiveness of inlining \
        (default %.02f, higher numbers mean more aggressive)"
      Clflags.default_inline_threshold, Arg.Status.empty

let mk_inline_toplevel f =
  "-inline-toplevel", Arg.String f,
    Printf.sprintf "<n>|<round>=<n>[,...]  Aggressiveness of inlining at \
      toplevel (higher numbers mean more aggressive)", Arg.Status.empty

let mk_inlining_report f =
  "-inlining-report", Arg.Unit f, " Emit `.<round>.inlining' file(s) (one per \
      round) showing the inliner's decisions", Arg.Status.empty
...
let mk_dump_pass f =
  ...
  Format.asprintf
    ...
    !Clflags.all_passes, Arg.Status.empty

let mk_o2 f =
  "-O2", Arg.Unit f, " Apply increased optimization for speed", Arg.Status.empty

let mk_o3 f =
  "-O3", Arg.Unit f, " Apply aggressive optimization for speed (may \
    significantly increase code size and compilation time)", Arg.Status.empty
...
let mk_rounds f =
  "-rounds", Arg.Int f,
    Printf.sprintf "<n>  Repeat tree optimization and inlining phases this \
      ...
      !Clflags.default_simplify_rounds, Arg.Status.empty
...
let mk_inline_max_unroll f =
  "-inline-max-unroll", Arg.String f,
    Printf.sprintf "<n>|<round>=<n>[,...]  Unroll recursive functions at most \
      ...
      Clflags.default_inline_max_unroll, Arg.Status.empty
...
let mk_inline_lifting_benefit f =
  ...
  Printf.sprintf "<n>|<round>=<n>[,...]  The benefit of lifting definitions \
    ...
    Clflags.default_inline_lifting_benefit, Arg.Status.empty
...
let mk_inline_branch_factor f =
  "-inline-branch-factor", Arg.String f,
    ...
    Clflags.default_inline_branch_factor, Arg.Status.empty

let mk_intf f =
  "-intf", Arg.String f, "<file>  Compile <file> as a .mli file", Arg.Status.empty

let mk_intf_suffix f =
  "-intf-suffix", Arg.String f,
  "<string>  Suffix for interface files (default: .mli)", Arg.Status.empty

let mk_intf_suffix_2 f =
  "-intf_suffix", Arg.String f, "<string>  (deprecated) same as -intf-suffix", Arg.Status.empty

let mk_insn_sched f =
  "-insn-sched", Arg.Unit f,
  Printf.sprintf " Run the instruction scheduling pass%s"
    (if Clflags.insn_sched_default then " (default)" else ""), Arg.Status.empty

let mk_no_insn_sched f =
  "-no-insn-sched", Arg.Unit f,
  Printf.sprintf " Do not run the instruction scheduling pass%s"
    (if not Clflags.insn_sched_default then " (default)" else ""), Arg.Status.empty

let mk_keep_docs f =
  "-keep-docs", Arg.Unit f, " Keep documentation strings in .cmi files", Arg.Status.empty

let mk_no_keep_docs f =
  "-no-keep-docs", Arg.Unit f,
  " Do not keep documentation strings in .cmi files (default)", Arg.Status.empty

let mk_keep_locs f =
  "-keep-locs", Arg.Unit f, " Keep locations in .cmi files (default)", Arg.Status.empty

let mk_no_keep_locs f =
  "-no-keep-locs", Arg.Unit f, " Do not keep locations in .cmi files", Arg.Status.empty

let mk_labels f =
  "-labels", Arg.Unit f, " Use commuting label mode", Arg.Status.empty

let mk_linkall f =
  "-linkall", Arg.Unit f, " Link all modules, even unused ones", Arg.Status.empty

let mk_linscan f =
  "-linscan", Arg.Unit f, " Use the linear scan register allocator", Arg.Status.empty

let mk_make_runtime f =
  "-make-runtime", Arg.Unit f,
  " Build a runtime system with given C objects and libraries", Arg.Status.empty

let mk_make_runtime_2 f =
  "-make_runtime", Arg.Unit f, " (deprecated) same as -make-runtime", Arg.Status.empty
...
let mk_inline_max_depth f =
  "-inline-max-depth", Arg.String f,
    Printf.sprintf "<n>|<round>=<n>[,...]  Maximum depth of search for \
      ...
      Clflags.default_inline_max_depth, Arg.Status.empty

let mk_modern f =
  "-modern", Arg.Unit f, " (deprecated) same as -labels", Arg.Status.empty

let mk_alias_deps f =
  "-alias-deps", Arg.Unit f,
  " Do record dependencies for module aliases", Arg.Status.empty

let mk_no_alias_deps f =
  "-no-alias-deps", Arg.Unit f,
  " Do not record dependencies for module aliases", Arg.Status.empty

let mk_app_funct f =
  "-app-funct", Arg.Unit f, " Activate applicative functors", Arg.Status.empty

let mk_no_app_funct f =
  "-no-app-funct", Arg.Unit f, " Deactivate applicative functors", Arg.Status.empty

let mk_no_check_prims f =
  "-no-check-prims", Arg.Unit f, " Do not check runtime for primitives", Arg.Status.empty

let mk_no_float_const_prop f =
  "-no-float-const-prop", Arg.Unit f,
  " Deactivate constant propagation for floating-point operations", Arg.Status.empty

let mk_noassert f =
  "-noassert", Arg.Unit f, " Do not compile assertion checks", Arg.Status.empty

let mk_noautolink_byt f =
  "-noautolink", Arg.Unit f,
  " Do not automatically link C libraries specified in .cma files", Arg.Status.empty

let mk_noautolink_opt f =
  "-noautolink", Arg.Unit f,
  " Do not automatically link C libraries specified in .cmxa files", Arg.Status.empty

let mk_nodynlink f =
  "-nodynlink", Arg.Unit f,
  " Enable optimizations for code that will not be dynlinked", Arg.Status.empty

let mk_noinit f =
  "-noinit", Arg.Unit f,
  " Do not load any init file", Arg.Status.empty

let mk_nolabels f =
  "-nolabels", Arg.Unit f, " Ignore non-optional labels in types", Arg.Status.empty

let mk_noprompt f =
  "-noprompt", Arg.Unit f, " Suppress all prompts", Arg.Status.empty

let mk_nopromptcont f =
  "-nopromptcont", Arg.Unit f,
  " Suppress prompts for continuation lines of multi-line inputs", Arg.Status.empty

let mk_nostdlib f =
  "-nostdlib", Arg.Unit f,
  " Do not add default directory to the list of include directories", Arg.Status.empty

let mk_nocwd f =
  "-nocwd", Arg.Unit f,
  " Do not implicitly add the current directory to the load path", Arg.Status.empty

let mk_no_unbox_free_vars_of_closures f =
  "-no-unbox-free-vars-of-closures", Arg.Unit f,
  " Do not unbox variables that will appear inside function closures", Arg.Status.empty

let mk_no_unbox_specialised_args f =
  "-no-unbox-specialised-args", Arg.Unit f,
  " Do not unbox arguments to which functions have been specialised", Arg.Status.empty

let mk_o f =
  "-o", Arg.String f, "<file>  Set output file name to <file>", Arg.Status.empty

let mk_open f =
  "-open", Arg.String f, "<module>  Opens the module <module> before typing", Arg.Status.empty

let mk_output_obj f =
  "-output-obj", Arg.Unit f, " Output an object file instead of an executable", Arg.Status.empty

let mk_output_complete_obj f =
  "-output-complete-obj", Arg.Unit f,
  " Output an object file, including runtime, instead of an executable", Arg.Status.empty

let mk_output_complete_exe f =
  "-output-complete-exe", Arg.Unit f,
  " Output a self-contained executable, including runtime and C stubs", Arg.Status.empty

let mk_p f =
  "-p", Arg.Unit f, " (no longer supported)", Arg.Status.empty

let mk_pack_byt f =
  "-pack", Arg.Unit f, " Package the given .cmo files into one .cmo", Arg.Status.empty

let mk_pack_opt f =
  "-pack", Arg.Unit f, " Package the given .cmx files into one .cmx", Arg.Status.empty

let mk_pp f =
  "-pp", Arg.String f, "<command>  Pipe sources through preprocessor <command>", Arg.Status.empty

let mk_ppx f =
  "-ppx", Arg.String f,
  "<command>  Pipe abstract syntax trees through preprocessor <command>", Arg.Status.empty

let mk_plugin f =
  "-plugin", Arg.String f,
  "<plugin>  (no longer supported)", Arg.Status.empty

let mk_principal f =
  "-principal", Arg.Unit f, " Check principality of type inference", Arg.Status.empty

let mk_no_principal f =
  "-no-principal", Arg.Unit f,
  " Do not check principality of type inference (default)", Arg.Status.empty

let mk_rectypes f =
  "-rectypes", Arg.Unit f, " Allow arbitrary recursive types", Arg.Status.empty

let mk_no_rectypes f =
  "-no-rectypes", Arg.Unit f,
  " Do not allow arbitrary recursive types (default)", Arg.Status.empty

let mk_remove_unused_arguments f =
  "-remove-unused-arguments", Arg.Unit f,
  " Remove unused function arguments", Arg.Status.empty

let mk_runtime_variant f =
  "-runtime-variant", Arg.String f,
  "<str>  Use the <str> variant of the run-time system", Arg.Status.empty

let mk_with_runtime f =
  "-with-runtime", Arg.Unit f,
  "Include the runtime system in the generated program (default)", Arg.Status.empty

let mk_without_runtime f =
  "-without-runtime", Arg.Unit f,
  "Do not include the runtime system in the generated program.", Arg.Status.empty

let mk_S f =
  "-S", Arg.Unit f, " Keep intermediate assembly file", Arg.Status.empty

let mk_safe_string =
  "-safe-string", Arg.Unit (fun () -> ()),
  " (default unconditionally since 5.0)", Arg.Status.empty

let mk_safer_matching f =
  "-safer-matching", Arg.Unit f,
  " Do not use type information to optimize pattern-matching", Arg.Status.empty

let mk_shared f =
  "-shared", Arg.Unit f, " Produce a dynlinkable plugin", Arg.Status.empty

let mk_short_paths f =
  "-short-paths", Arg.Unit f, " Shorten paths in types", Arg.Status.empty

let mk_stdin f =
  "-stdin", Arg.Unit f, " Read script from standard input", Arg.Status.empty

let mk_no_strict_sequence f =
  "-no-strict-sequence", Arg.Unit f,
  " Left-hand part of a sequence need not have type unit (default)", Arg.Status.empty

let mk_strict_sequence f =
  "-strict-sequence", Arg.Unit f,
  " Left-hand part of a sequence must have type unit", Arg.Status.empty

let mk_thread f =
  "-thread", Arg.Unit f,
  " same as -I +threads", { Arg.Status.empty with deprecated_version = Some "TODO" }

let mk_dtimings f =
  "-dtimings", Arg.Unit f, " Print timings information for each pass", Arg.Status.empty

let mk_dprofile f =
  "-dprofile", Arg.Unit f, Profile.options_doc, Arg.Status.empty

let mk_unbox_closures f =
  "-unbox-closures", Arg.Unit f,
  " Pass free variables via specialised arguments rather than closures", Arg.Status.empty
...
let mk_unbox_closures_factor f =
  ...
  (Printf.sprintf "<n > 0>  Scale the size threshold above which \
      ...
      function (default %d)"
    Clflags.default_unbox_closures_factor), Arg.Status.empty

let mk_unboxed_types f =
  "-unboxed-types", Arg.Unit f,
  " unannotated unboxable types will be unboxed", Arg.Status.empty

let mk_no_unboxed_types f =
  "-no-unboxed-types", Arg.Unit f,
  " unannotated unboxable types will not be unboxed (default)", Arg.Status.empty

let mk_unsafe f =
  "-unsafe", Arg.Unit f,
  " Do not compile bounds checking on array and string access", Arg.Status.empty

let mk_unsafe_string =
 let err () =
   raise (Arg.Bad "-unsafe-string is not available since OCaml 5.0")
 in
 "-unsafe-string", Arg.Unit err, " (option not available)", Arg.Status.empty

let mk_use_runtime f =
  "-use-runtime", Arg.String f,
  "<file>  Generate bytecode for the given runtime system", Arg.Status.empty

let mk_use_runtime_2 f =
  "-use_runtime", Arg.String f,
  "<file>  same as -use-runtime", Arg.Status.empty

let mk_v f =
  "-v", Arg.Unit f,
  " Print compiler version and location of standard library and exit", Arg.Status.empty

let mk_verbose f =
  "-verbose", Arg.Unit f, " Print calls to external commands", Arg.Status.empty

let mk_version f =
  "-version", Arg.Unit f, " Print version and exit", Arg.Status.empty

let mk__version f =
  "--version", Arg.Unit f, " Print version and exit", Arg.Status.empty

let mk_no_version f =
  "-no-version", Arg.Unit f, " Do not print version at startup", Arg.Status.empty

let mk_vmthread f =
  "-vmthread", Arg.Unit f,
  "  (no longer supported)", Arg.Status.empty

let mk_vnum f =
  "-vnum", Arg.Unit f, " Print version number and exit", Arg.Status.empty
...
let mk_w f =
  ...
  \     default setting is %S" Warnings.defaults_w, Arg.Status.empty
...
let mk_warn_error f =
  ...
  \     Default setting is %S" Warnings.defaults_warn_error, Arg.Status.empty

let mk_warn_help f =
  "-warn-help", Arg.Unit f, " Show description of warning numbers", Arg.Status.empty
...
let mk_color f =
  ...
  \  be set through the OCAML_COLOR environment variable.", Arg.Status.empty
...
let mk_error_style f =
  ...
  Printf.sprintf
    ...
    \  If the option is not specified, these setting can alternatively\n\
    \  be set through the OCAML_ERROR_STYLE environment variable.", Arg.Status.empty

let mk_where f =
  "-where", Arg.Unit f, " Print location of standard library and exit", Arg.Status.empty

let mk_nopervasives f =
  "-nopervasives", Arg.Unit f, " (undocumented)", Arg.Status.empty
...
let mk_use_prims f =
  "-use-prims", Arg.String f, "<file>  (undocumented)", Arg.Status.empty

let mk_dump_into_file f =
  "-dump-into-file", Arg.Unit f, " dump output like -dlambda into <target>.dump", Arg.Status.empty

let mk_dump_dir f =
  "-dump-dir", Arg.String f,
  "<dir> dump output like -dlambda into <dir>/<target>.dump", Arg.Status.empty

let mk_dparsetree f =
  "-dparsetree", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dtypedtree f =
  "-dtypedtree", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dshape f =
  "-dshape", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_drawlambda f =
  "-drawlambda", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dno_unique_ids f =
  "-dno-unique-ids", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dunique_ids f =
  "-dunique-ids", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dno_locations f =
  "-dno-locations", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dlocations f =
  "-dlocations", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dsource f =
  "-dsource", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dlambda f =
  "-dlambda", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_drawclambda f =
  "-drawclambda", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dclambda f =
  "-dclambda", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dflambda f =
  "-dflambda", Arg.Unit f, " Print Flambda terms", Arg.Status.empty

let mk_drawflambda f =
  "-drawflambda", Arg.Unit f, " Print Flambda terms after closure conversion", Arg.Status.empty

let mk_dflambda_invariants f =
  "-dflambda-invariants", Arg.Unit f, " Check Flambda invariants \
      around each pass", Arg.Status.empty
...
let mk_dflambda_let f =
  "-dflambda-let", Arg.Int f, "<stamp>  Print when the given Flambda [Let] \
      is created", Arg.Status.empty

let mk_dflambda_verbose f =
  "-dflambda-verbose", Arg.Unit f, " Print Flambda terms including around \
      each pass", Arg.Status.empty

let mk_dinstr f =
  "-dinstr", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dcamlprimc f =
  "-dcamlprimc", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dcmm_invariants f =
  "-dcmm-invariants", Arg.Unit f, " Extra sanity checks on Cmm", Arg.Status.empty

let mk_dcmm f =
  "-dcmm", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dsel f =
  "-dsel", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dcombine f =
  "-dcombine", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dcse f =
  "-dcse", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dlive f =
  "-dlive", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dspill f =
  "-dspill", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dsplit f =
  "-dsplit", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dinterf f =
  "-dinterf", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dprefer f =
  "-dprefer", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dalloc f =
  "-dalloc", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dreload f =
  "-dreload", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dscheduling f =
  "-dscheduling", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dlinear f =
  "-dlinear", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dinterval f =
  "-dinterval", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_dstartup f =
  "-dstartup", Arg.Unit f, " (undocumented)", Arg.Status.empty

let mk_opaque f =
  "-opaque", Arg.Unit f,
  " Does not generate cross-module optimization information\n\
  \     (reduces necessary recompilation on module change)", Arg.Status.empty
...
let mk_strict_formats f =
  ...
  \      to detect invalid formats so you can fix them.)", Arg.Status.empty
...
let mk_no_strict_formats f =
  ...
  \      and instead fix invalid formats.)", Arg.Status.empty
...
let mk_afl_instrument f =
  "-afl-instrument", Arg.Unit f, "Enable instrumentation for afl-fuzz", Arg.Status.empty

let mk_afl_inst_ratio f =
  "-afl-inst-ratio", Arg.Int f,
  "Configure percentage of branches instrumented\n\
  \     (advanced, see afl-fuzz docs for AFL_INST_RATIO)", Arg.Status.empty

let mk__ f =
  "-", Arg.String f,
  "<file>  Treat <file> as a file name (even if it starts with `-')", Arg.Status.empty
...
let options_with_command_line_syntax options r =
  ...
  List.map (fun (name, spec, doc, status) ->
    (name,
     options_with_command_line_syntax_inner r rest
       ~name_opt:(Some name) spec, doc, status)
  ) options
