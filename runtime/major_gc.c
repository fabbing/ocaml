...
static void mark_slice_darken(struct mark_stack* stk, value child,
                              intnat* work)
{
  const struct global_heap_state* heap_state = &caml_global_heap_state;
  uintnat* blocks_marked = &Caml_state->stat_blocks_marked;

  child = mark_slice_darken_only(child, work, blocks_marked, heap_state);
  if (child) {
